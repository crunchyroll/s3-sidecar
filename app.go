...
import (
	...
	"github.com/crunchyroll/s3-sidecar/controllers"
...
func (a *App) Initialize(configpath string) {
	...
	if err != nil {
		fmt.Printf("app failed to start due to invalid config. error: %+v, config-path: %s", err.Error(), configpath)
	...
	if err != nil {
		fmt.Printf("app failed to init logger. Error: %+v", err.Error())
		os.Exit(1) // kill the app
	}
...
func (a *App) initializeRoutes() {
	...
	ic := controllers.NewItemController(a.Configs.S3.Region, a.Configs.S3.Bucket, a.Logger)

	// ELB specific health check endpoint
	a.Router.HandleFunc("/_health", sc.SystemHealth).Methods("GET")
	// sh := handler.NewS3Handler(a.Configs.S3Config.Region, aa.Logger)
	a.Router.HandleFunc("/content/{asset}/{item}", ic.GetContent).Methods("GET")
}
